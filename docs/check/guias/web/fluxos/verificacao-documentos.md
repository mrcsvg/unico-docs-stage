...
Este método gera uma *promise* que ao ser resolvida, devolve um objeto que será utilizado para efetivamente abrir a câmera através do método `open`, que recebe como parâmetro as funções de `callback` configuradas [neste passo](#configurar-funções-de-callback).

:::info Vale lembrar
Para otimizar a abertura da câmera é possível separar as chamadas dos métodos `prepareSelfieCamera()` e `open()`.
Veja um exemplo nesta [POC feita em React com TypeScript](/recursos-adicionais/exemplos#projetos-web). 
:::
